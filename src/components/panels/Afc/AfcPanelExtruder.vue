...
export default class AfcPanelExtruder extends Mixins(BaseMixin, AfcMixin) {
    ...
    get activeToolheadExtruderName(): string | null {
        const printer = this.$store.state.printer ?? null
        if (!printer || typeof printer !== 'object') return null

        const toolhead = (printer as Record<string, unknown>)['toolhead']
        if (!toolhead || typeof toolhead !== 'object') return null

        const rawExtruder = (toolhead as Record<string, unknown>)['extruder']

        return this.normalizeExtruderName(rawExtruder)
    }
    ...
    get isActiveExtruder() {
        ...
        const activeToolheadExtruder = this.activeToolheadExtruderName
        if (activeToolheadExtruder && activeToolheadExtruder === normalized) return true
