...
export default class AfcPanelExtruder extends Mixins(BaseMixin, AfcMixin) {
    ...
    get isActiveAmsExtruder() {
        if (!this.isCurrentLaneLoadedByExtruder) return false
    ...
    get activeAmsUnitCandidates(): string[] {
        ...
        if (!this.isCurrentLaneLoadedByExtruder) return candidates

        const addCandidate = (rawUnit: unknown) => {
        ...
        addCandidate(this.currentLaneUnitName)

        for (const unitName of this.amsUnitNamesByExtruder) addCandidate(unitName)
